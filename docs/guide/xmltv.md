...
**_XMLTV**_ format. The format is based on the
[_XMLTV DTD_](https://github.com/XMLTV/xmltv/blob/master/xmltv.dtd).

### Example Usage

_See [example XMLTV document](#xmltv-example-document)._
...
document.ready(() => {
  console.log(document.query(`programmes[$int(start) > $int("${Date()}")]`))
})
.catch(console.error)
```
...
The `mediaxml/xmltv` module provides a canonical `Document` class that is
the root of the XMLTV XML implementation. The various other classes
provided by the `mediaxml/xmltv` module have accessors for properties
defined by XMLTV that give normalized values in a convenient manner such
as a start and stop dates,

#### `Document`

A class that represents an XMLTV XML document.
...
document.ready(() => {
  ...
  // computed `<AMS />` (Asset Management Service) node for this XMLTV metadata.
  console.log(document.metadata.ams)

  // computed `<App_Data />` for this XMLTV metadata node. If none could be found
...
<a name="xmltv-example-document"></a>
### Example XMLTV Document
